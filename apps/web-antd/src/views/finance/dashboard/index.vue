...
type TrendRangeValue = '3month' | '6month' | 'month' | 'year';
...
const selectedRangeLabel = computed(() =>
  isEnglish.value
    ? selectedRangeOption.value.labelEn
    : selectedRangeOption.value.labelZh,
);
...
const trendChartData = computed(() => {
  ...
  const bucketMap = new Map<string, { expense: number; income: number }>();
  ...
  const incomeCount = rangeTransactions.value.filter(
    (t) => t.type === 'income',
  ).length;
  const expenseCount = rangeTransactions.value.filter(
    (t) => t.type === 'expense',
  ).length;
...
const rangeTransactionCount = computed(
  () => trendChartData.value.totalTransactions,
);
...
const rangeIncomePercentage = computed(() => {
  ...
  return Math.min(
    100,
    Math.round((rangeIncome.value / rangeTotalAmount.value) * 100),
  );
});
const rangeExpensePercentage = computed(() => {
  ...
  return Math.min(
    100,
    Math.round((rangeExpense.value / rangeTotalAmount.value) * 100),
  );
});

const expenseByCategory = computed(() => {
  const expenseTransactions = rangeTransactions.value.filter(
    (t) => t.type === 'expense',
  );
  ...
  const categoryMap = new Map<
    number,
    { amount: number; categoryId: number; categoryName: string; count: number }
  >();
  ...
  expenseTransactions.forEach((transaction) => {
    ...
    if (!categoryMap.has(categoryId)) {
      categoryMap.set(categoryId, {
        categoryId,
        categoryName:
          financeStore.getCategoryById(categoryId)?.name ||
          (english ? 'Uncategorized' : '未分类'),
        amount: 0,
        count: 0,
      });
    }
  ...
  return [...categoryMap.values()]
    .map((item) => {
      ...
      const percentage =
        rangeExpense.value === 0
          ? 0
          : Math.round((amount / rangeExpense.value) * 100);
      return {
        ...item,
        amount,
        percentage,
      ...
      tooltip: {
        ...
        formatter: (items: any[]) => {
          ...
          const rows = items
            .map(
              (item) =>
                `${item.marker}${item.seriesName}: ${formatCurrency(Number(item.data ?? 0))}`,
            )
            .join('<br />');
...
const changeLanguage = (lang: string) => {
  ...
  notification.success({
    ...
    description:
      lang === 'en-US' ? 'Language switched to English' : '语言已切换为中文',
  });
};
...
const toggleTheme = () => {
  ...
  notification.info({
    ...
    description: isEnglish.value
      ? `Switched to ${isDark.value ? 'Dark' : 'Light'} theme`
      : `已切换到${isDark.value ? '深色' : '浅色'}主题`,
  });
};
  ...
  <div class="p-6">
    <div class="mb-6">
      ...
      <p class="text-gray-600">
        {{
          isEnglish
            ? 'Comprehensive financial data overview and real-time monitoring'
            : '智能财务数据概览与实时监控'
        }}
      </p>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="mb-1 text-sm text-gray-500">{{ metric.title }}</p>
            <p class="text-2xl font-bold" :class="metric.color">
              {{ metric.value }}
            </p>
            <p
              class="text-xs"
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <Card class="lg:col-span-2" title="📈 收支趋势分析">
        <div class="space-y-6">
          ...
          <div
            v-if="trendChartData.hasData"
            class="rounded-lg bg-gray-50 p-2 dark:bg-gray-800"
          >
            <EchartsUI ref="trendChartRef" height="320px" />
          </div>
          <div
            v-else
            class="flex h-80 flex-col items-center justify-center rounded-lg bg-gray-50 text-gray-500 dark:bg-gray-800"
          >
            ...
            <p class="text-sm text-gray-400">
              {{
                isEnglish
                  ? 'Record transactions or adjust the range to view trends'
                  : '新增交易或更换时间范围后可查看趋势'
              }}
            </p>
          </div>
          ...
          <div class="space-y-6">
            <div>
              <div class="mb-2 flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700">
                  ...
                  {{
                    isEnglish
                      ? `Income (${selectedRangeLabel})`
                      : `${selectedRangeLabel}收入`
                  }}
                </span>
            ...
            <div>
              <div class="mb-2 flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700">
                  ...
                  {{
                    isEnglish
                      ? `Expense (${selectedRangeLabel})`
                      : `${selectedRangeLabel}支出`
                  }}
                </span>
              ...
              >
                <div
                  class="h-3 rounded-full bg-gradient-to-r from-red-400 to-red-600 transition-all duration-500"
            ...
            <div class="flex items-center justify-between border-t pt-4">
              <span class="text-sm font-medium text-gray-700">
                ...
                {{
                  isEnglish
                    ? `Net Income (${selectedRangeLabel})`
                    : `${selectedRangeLabel}净收入`
                }}
              </span>
              <span
                class="text-lg font-bold"
                :class="
                  rangeNetIncome >= 0 ? 'text-purple-600' : 'text-red-600'
                "
              >
            ...
            <div class="grid grid-cols-3 gap-4 border-t pt-4">
              ...
              <div class="text-center">
                ...
                <div class="text-xs text-gray-500">
                  {{ isEnglish ? 'Expense Records' : '支出笔数' }}
                </div>
              </div>
            </div>
          </div>
      ...
      <Card title="🥧 支出分类分布">
        <div class="space-y-4">
          <div
            v-if="categoryChartData.hasData"
            class="rounded-lg bg-gray-50 p-2 dark:bg-gray-800"
          >
          ...
          <div
            v-else
            class="flex h-64 flex-col items-center justify-center rounded-lg bg-gray-50 text-gray-500 dark:bg-gray-800"
          >
            <div class="mb-2 text-4xl">🥱</div>
            <p class="font-medium">
              {{
                isEnglish
                  ? 'No expense records for the selected range'
                  : '选定范围内暂无支出记录'
              }}
            </p>
            <p class="text-sm text-gray-400">
              {{
                isEnglish
                  ? 'Record expenses to view category breakdown'
                  : '新增支出后即可查看分类占比'
              }}
            </p>
          </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <Card title="🕒 最近交易记录">
        <template #extra>
          <Button type="link" @click="$router.push('/finance/transactions')">
            查看全部
          </Button>
        </template>
        <div v-if="recentTransactions.length === 0" class="py-8 text-center">
          <div class="mb-4 text-6xl">📝</div>
          <p class="mb-4 text-gray-500">暂无交易记录</p>
      ...
      <Card title="🏦 账户余额">
        <template #extra>
          <Button type="link" @click="$router.push('/finance/accounts')">
            管理账户
          </Button>
        </template>
        <div v-if="accounts.length === 0" class="py-8 text-center">
          <div class="mb-4 text-6xl">🏦</div>
          <p class="mb-4 text-gray-500">暂无账户信息</p>
    ...
    <Modal
      ...
      :title="isEnglish ? '💰 Quick Add Income' : '💰 快速添加收入'"
      @ok="submitIncome"
      width="600px"
    >
      <Form :model="quickIncomeForm" layout="vertical">
        ...
        <Form.Item :label="isEnglish ? 'Account' : '收入账户'" required>
          <Select
            ...
            :placeholder="isEnglish ? 'Select account' : '选择账户'"
            size="large"
          >
            <Select.Option
        ...
        <Form.Item :label="isEnglish ? 'Date' : '日期'" required>
          <div class="space-y-2">
            <div class="mb-2 flex space-x-2">
              ...
              <Button size="small" @click="setIncomeDate('yesterday')">
                昨天
              </Button>
    ...
    >
      <Form :model="quickExpenseForm" layout="vertical">
        ...
        <Form.Item :label="isEnglish ? 'Date' : '日期'" required>
          <div class="space-y-2">
            <div class="mb-2 flex space-x-2">
              <Button size="small" @click="setExpenseDate('today')">
                今天
              </Button>
              <Button size="small" @click="setExpenseDate('yesterday')">
                昨天
              </Button>
              <Button size="small" @click="setExpenseDate('week')">本周</Button>
              <Button size="small" @click="setExpenseDate('month')">
                本月
              </Button>
            </div>
            <DatePicker
    ...
    <Modal
      ...
      :title="isEnglish ? '🏦 Add Account' : '🏦 添加账户'"
      @ok="submitAccount"
    >
      <Form :model="quickAccountForm" layout="vertical">
        <Form.Item :label="isEnglish ? 'Account Name' : '账户名称'" required>
          <Input
            v-model:value="quickAccountForm.name"
            :placeholder="isEnglish ? 'Enter account name' : '请输入账户名称'"
          />
        </Form.Item>
        <Form.Item :label="isEnglish ? 'Account Type' : '账户类型'">
          <Select v-model:value="quickAccountForm.type" style="width: 100%">
            <Select.Option value="savings">
              {{ isEnglish ? 'Savings Account' : '储蓄账户' }}
            </Select.Option>
            <Select.Option value="checking">
              {{ isEnglish ? 'Checking Account' : '支票账户' }}
            </Select.Option>
            <Select.Option value="credit">
              {{ isEnglish ? 'Credit Card' : '信用卡' }}
            </Select.Option>
            <Select.Option value="investment">
              {{ isEnglish ? 'Investment Account' : '投资账户' }}
            </Select.Option>
            <Select.Option value="ewallet">
              {{ isEnglish ? 'E-Wallet' : '电子钱包' }}
            </Select.Option>
          </Select>
        </Form.Item>
        <Form.Item :label="isEnglish ? 'Initial Balance' : '初始余额'">
          <InputNumber
            v-model:value="quickAccountForm.initialBalance"
            :precision="2"
            style="width: 100%"
            :placeholder="
              isEnglish ? 'Enter initial balance' : '请输入初始余额'
            "
          />
        </Form.Item>
      </Form>
