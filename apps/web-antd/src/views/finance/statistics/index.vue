<template>
  <div class="p-6">
    <div class="mb-6">
      <h1 class="text-3xl font-bold text-gray-900 mb-2">📊 财务统计分析</h1>
      <p class="text-gray-600">多维度智能分析您的财务数据</p>
    </div>

    <!-- 时间维度选择器 -->
    <Card class="mb-6">
      <Tabs v-model:activeKey="timeDimension" @change="handleDimensionChange">
        <TabPane key="monthly" tab="📅 月度分析">
          <div class="flex items-center space-x-4 py-2">
            ...
            <DatePicker
              ...
              style="width: 200px"
            />
            <Button @click="nextPeriod" :disabled="!canGoNext">
              →
            </Button>
        ...
        <TabPane key="quarterly" tab="📊 季度分析">
          <div class="flex items-center space-x-4 py-2">
            ...
            <Select v-model:value="selectedQuarter" style="width: 200px" @change="handleQuarterChange">
              <Select.Option v-for="quarter in availableQuarters" :key="quarter.value" :value="quarter.value">
                {{ quarter.label }}
              </Select.Option>
            </Select>
            <Button @click="nextPeriod" :disabled="!canGoNext">
        ...
        <TabPane key="yearly" tab="📈 年度分析">
          <div class="flex items-center space-x-4 py-2">
            ...
            <Select v-model:value="selectedYear" style="width: 200px" @change="handleYearChange">
    ...
    <Card class="mb-6" title="💡 智能洞察">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div v-for="insight in smartInsights" :key="insight.type" class="p-4 rounded-lg border" :class="insight.alertLevel">
          ...
          <div class="flex items-center justify-between">
            <span class="text-xl font-bold" :class="insight.valueClass">{{ insight.value }}</span>
            <span v-if="insight.trend" class="text-sm" :class="insight.trendClass">
      ...
      <Card class="text-center hover:shadow-lg transition-shadow">
        <div class="space-y-2">
          ...
          <p class="text-2xl font-bold text-green-600">
            ¥{{ currentPeriodData.totalIncome.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
          </p>
          <p v-if="comparisonData.incomeTrend !== null" class="text-xs" :class="comparisonData.incomeTrend >= 0 ? 'text-green-600' : 'text-red-600'">
      ...
      <Card class="text-center hover:shadow-lg transition-shadow">
        <div class="space-y-2">
          ...
          <p class="text-2xl font-bold text-red-600">
            ¥{{ currentPeriodData.totalExpense.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
          </p>
          <p v-if="comparisonData.expenseTrend !== null" class="text-xs" :class="comparisonData.expenseTrend <= 0 ? 'text-green-600' : 'text-red-600'">
      ...
      <Card class="text-center hover:shadow-lg transition-shadow">
        <div class="space-y-2">
          ...
          <p class="text-2xl font-bold text-blue-600">
            ¥{{ currentPeriodData.avgAmount.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
          </p>
          <p class="text-xs text-gray-500">支出笔数: {{ currentPeriodData.expenseCount }}</p>
        </div>
      </Card>
      <Card class="text-center hover:shadow-lg transition-shadow">
        <div class="space-y-2">
          ...
          <p class="text-2xl font-bold text-orange-600">
            ¥{{ currentPeriodData.maxExpense.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
          </p>
          <p v-if="currentPeriodData.maxExpenseCategory" class="text-xs text-gray-500">
    ...
    <Card class="mb-6" title="📈 对比分析">
      <Tabs v-model:activeKey="comparisonType">
        <TabPane key="mom" tab="环比分析">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="p-4 bg-blue-50 rounded-lg">
              <p class="text-sm text-gray-600 mb-2">支出环比</p>
              <p class="text-2xl font-bold" :class="comparisonData.momExpense < 0 ? 'text-green-600' : 'text-red-600'">
                {{ comparisonData.momExpense >= 0 ? '+' : '' }}{{ comparisonData.momExpense }}%
              </p>
            </div>
            <div class="p-4 bg-green-50 rounded-lg">
              <p class="text-sm text-gray-600 mb-2">收入环比</p>
              <p class="text-2xl font-bold" :class="comparisonData.momIncome >= 0 ? 'text-green-600' : 'text-red-600'">
                {{ comparisonData.momIncome >= 0 ? '+' : '' }}{{ comparisonData.momIncome }}%
              </p>
            </div>
            <div class="p-4 bg-purple-50 rounded-lg">
              <p class="text-sm text-gray-600 mb-2">交易笔数环比</p>
              <p class="text-2xl font-bold text-purple-600">
                {{ comparisonData.momCount >= 0 ? '+' : '' }}{{ comparisonData.momCount }}%
              </p>
            </div>
          </div>
        </TabPane>
        <TabPane key="yoy" tab="同比分析">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="p-4 bg-blue-50 rounded-lg">
              <p class="text-sm text-gray-600 mb-2">支出同比</p>
              <p class="text-2xl font-bold" :class="comparisonData.yoyExpense < 0 ? 'text-green-600' : 'text-red-600'">
                {{ comparisonData.yoyExpense >= 0 ? '+' : '' }}{{ comparisonData.yoyExpense }}%
              </p>
            </div>
            <div class="p-4 bg-green-50 rounded-lg">
              <p class="text-sm text-gray-600 mb-2">收入同比</p>
              <p class="text-2xl font-bold" :class="comparisonData.yoyIncome >= 0 ? 'text-green-600' : 'text-red-600'">
                {{ comparisonData.yoyIncome >= 0 ? '+' : '' }}{{ comparisonData.yoyIncome }}%
              </p>
            </div>
            <div class="p-4 bg-purple-50 rounded-lg">
              <p class="text-sm text-gray-600 mb-2">交易笔数同比</p>
              <p class="text-2xl font-bold text-purple-600">
                {{ comparisonData.yoyCount >= 0 ? '+' : '' }}{{ comparisonData.yoyCount }}%
              </p>
            </div>
          </div>
        </TabPane>
    ...
    <Card class="mb-6" title="💪 分类健康度评分">
      <div class="space-y-3">
        <div v-for="health in categoryHealthScores" :key="health.categoryId" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
          <div class="flex items-center space-x-3">
            <span class="text-2xl">{{ health.icon }}</span>
            <div>
              <p class="font-semibold">{{ health.categoryName }}</p>
              <p class="text-xs text-gray-500">
                ¥{{ health.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }} | {{ health.count }}笔
              </p>
            </div>
          </div>
          <div class="flex items-center space-x-4">
            <div class="w-32">
              <Progress :percent="health.percentage" :status="health.status" :stroke-color="health.color" />
            </div>
    ...
    <Card v-if="anomalies.length > 0" title="⚠️ 异常交易检测" class="mb-6">
      ...
      />
      <div class="space-y-2">
        <div v-for="anomaly in anomalies" :key="anomaly.id" class="p-3 border-l-4 border-orange-500 bg-orange-50 rounded">
          <div class="flex items-center justify-between">
            <div>
              <p class="font-semibold">{{ anomaly.description }}</p>
              <p class="text-sm text-gray-600">{{ anomaly.date }} | {{ anomaly.category }}</p>
            </div>
            <div class="text-right">
              <p class="text-lg font-bold text-red-600">-¥{{ anomaly.amount.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}</p>
    ...
    <Card title="📋 月度数据表格" class="mb-6">
      <div class="mb-4 flex justify-between items-center">
        <div class="flex items-center space-x-4">
          <Select v-model:value="tableViewYear" style="width: 120px" @change="handleTableYearChange">
            <Select.Option v-for="year in availableYears" :key="year" :value="year">
              {{ year }}年
            </Select.Option>
          </Select>
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'income'">
            <span class="text-green-600 font-semibold">
              ¥{{ record.income.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
            </span>
          </template>
          <template v-else-if="column.key === 'expense'">
            <span class="text-red-600 font-semibold">
              ¥{{ record.expense.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
            </span>
          </template>
          <template v-else-if="column.key === 'net'">
            <span :class="record.net >= 0 ? 'text-green-600' : 'text-red-600'" class="font-bold">
              {{ record.net >= 0 ? '+' : '' }}¥{{ record.net.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
            </span>
          </template>
          <template v-else-if="column.key === 'savingsRate'">
            <span :class="record.savingsRate >= 30 ? 'text-green-600' : record.savingsRate >= 10 ? 'text-orange-600' : 'text-red-600'">
              {{ record.savingsRate }}%
            </span>
          </template>
        </template>
      ...
      <Table
        ...
        :dataSource="monthlyCategoryData"
        :pagination="false"
        :scroll="{ x: 1200 }"
        bordered
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.dataIndex !== 'month' && column.dataIndex !== 'total'">
            <div v-if="record[column.dataIndex]">
              <div class="font-semibold">¥{{ record[column.dataIndex].amount.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}</div>
              <div class="text-xs text-gray-500">{{ record[column.dataIndex].percentage }}%</div>
            </div>
            <div v-else class="text-gray-400">-</div>
          </template>
          <template v-else-if="column.key === 'total'">
            <span class="text-red-600 font-bold">
              ¥{{ record.total.toLocaleString('zh-CN', { minimumFractionDigits: 2 }) }}
            </span>
          </template>
        </template>
      </Table>
...
import { Card, Tabs, TabPane, Button, DatePicker, Select, RangePicker, Badge, Progress, Alert, Table, Radio, notification } from 'ant-design-vue';
...
// 时间维度
const timeDimension = ref<'monthly' | 'quarterly' | 'yearly' | 'custom'>('monthly');
...
const getCurrentPeriodTransactions = () => {
  ...
  switch (timeDimension.value) {
    ...
    case 'quarterly':
      ...
      startDate = dayjs(`${year}-${(parseInt(quarter) - 1) * 3 + 1}-01`).startOf('month');
...
const currentPeriodData = computed(() => {
  ...
  const income = transactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amountInBase, 0);
  const expense = transactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amountInBase, 0);
  const expenseTransactions = transactions.filter(t => t.type === 'expense');

  const maxExpenseTransaction = expenseTransactions.length > 0
    ? expenseTransactions.reduce((max, t) => t.amountInBase > max.amountInBase ? t : max)
    : null;
  ...
  return {
    ...
    avgAmount: expenseTransactions.length > 0 ? expense / expenseTransactions.length : 0,
...
const smartInsights = computed(() => {
  ...
  // 洞察2: 高频分类
  const categoryStats = new Map<number, { count: number; amount: number }>();
...
const categoryHealthScores = computed(() => {
  const categoryStats = new Map<number, { count: number; amount: number }>();
  const transactions = getCurrentPeriodTransactions().filter(t => t.type === 'expense' && t.categoryId);
  ...
  return Array.from(categoryStats.entries())
    .map(([categoryId, stat]) => {
      ...
      const percentage = totalExpense > 0 ? (stat.amount / totalExpense * 100) : 0;
...
const anomalies = computed(() => {
  const transactions = getCurrentPeriodTransactions().filter(t => t.type === 'expense');
...
const previousPeriod = () => {
  switch (timeDimension.value) {
    ...
    case 'quarterly':
      const [year, quarter] = selectedQuarter.value.split('-Q');
      const prevQ = parseInt(quarter) - 1;
      if (prevQ < 1) {
        selectedQuarter.value = `${parseInt(year) - 1}-Q4`;
      } else {
        selectedQuarter.value = `${year}-Q${prevQ}`;
      }
      break;
    case 'yearly':
      selectedYear.value--;
...
const nextPeriod = () => {
  switch (timeDimension.value) {
    ...
    case 'quarterly':
      ...
      if (nextQ > 4) {
        selectedQuarter.value = `${parseInt(year) + 1}-Q1`;
      } else {
        selectedQuarter.value = `${year}-Q${nextQ}`;
      }
      break;
    case 'yearly':
      selectedYear.value++;
...
const tableViewMode = ref<'summary' | 'category'>('summary');

// 汇总表格列定义
const summaryTableColumns = [
  { title: '月份', dataIndex: 'month', key: 'month', width: 100, fixed: 'left' },
  { title: '收入', dataIndex: 'income', key: 'income', width: 150, align: 'right' },
  { title: '支出', dataIndex: 'expense', key: 'expense', width: 150, align: 'right' },
  { title: '净收入', dataIndex: 'net', key: 'net', width: 150, align: 'right' },
  { title: '交易笔数', dataIndex: 'count', key: 'count', width: 100, align: 'center' },
  { title: '储蓄率', dataIndex: 'savingsRate', key: 'savingsRate', width: 100, align: 'center' },
];
...
const categoryTableColumns = computed(() => {
  ...
  columns.push({ title: '总计', dataIndex: 'total', key: 'total', width: 150, align: 'right', fixed: 'right' });
...
const monthlyCategoryData = computed(() => {
  ...
  for (let month = 1; month <= 12; month++) {
    const monthStr = `${year}-${String(month).padStart(2, '0')}`;
    const monthTransactions = financeStore.transactions.filter(t => {
      return dayjs(t.transactionDate).format('YYYY-MM') === monthStr && t.type === 'expense';
    });
    ...
    expenseCategories.forEach(cat => {
      ...
      if (catExpense > 0) {
        const total = monthTransactions.reduce((sum, t) => sum + t.amountInBase, 0);
        const percentage = total > 0 ? Math.round((catExpense / total) * 100) : 0;
        row[`cat_${cat.id}`] = {
          amount: catExpense,
          percentage
        };
  ...
  expenseCategories.forEach(cat => {
    ...
    if (catTotal > 0) {
      ...
      const percentage = grandTotal > 0 ? Math.round((catTotal / grandTotal) * 100) : 0;
...
const exportMonthlyTable = () => {
  try {
    const data = tableViewMode.value === 'summary' ? monthlyTableData.value : monthlyCategoryData.value;
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, `${tableViewYear.value}年数据`);
    XLSX.writeFile(wb, `财务统计_${tableViewYear.value}年_${tableViewMode.value === 'summary' ? '汇总' : '分类'}.xlsx`);
