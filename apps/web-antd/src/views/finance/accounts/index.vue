...
const accountTransactions = computed(() => {
  ...
  return financeStore.transactions.filter(
    (t) => t.accountId === currentAccount.value.id && !t.isDeleted,
  );
});
...
const editAccount = (account: any) => {
  ...
  accountForm.value = {
    ...
    currency: ['CNY', 'EUR', 'GBP', 'HKD', 'JPY', 'KRW', 'USD'].includes(
      account.currency,
    )
      ? account.currency
      : 'CUSTOM',
...
const submitTransfer = async () => {
  try {
    ...
    const fromAccount = financeStore.getAccountById(
      Number(transferForm.value.fromAccount),
    );
    const toAccount = financeStore.getAccountById(
      Number(transferForm.value.toAccount),
    );
...
<template>
  <div class="p-6">
    <div class="mb-6">
      <h1 class="mb-2 text-3xl font-bold text-gray-900">🏦 账户管理</h1>
    ...
    <div v-if="accounts.length === 0" class="py-12 text-center">
      ...
      <h3 class="mb-2 text-xl font-medium text-gray-800">暂无账户信息</h3>
      <p class="mb-6 text-gray-500">添加您的银行账户、电子钱包等开始管理财务</p>
      ...
      >
          ...
          <Dropdown :trigger="['click']">
            <template #overlay>
              <Menu>
                ...
                <Menu.Item @click="deleteAccount(account)" class="text-red-600">
                  🗑️ 删除
                </Menu.Item>
              </Menu>
            </template>
        ...
        <div class="space-y-4">
          <div class="text-center">
            ...
            <p class="mt-1 text-xs text-blue-500">{{ account.currency }}</p>
          </div>
          ...
          <div class="flex space-x-2">
            ...
            <Button size="small" block @click="viewDetails(account)">
              📊 明细
            </Button>
          </div>
        </div>
      ...
      <Form ref="formRef" :model="accountForm" :rules="rules" layout="vertical">
        ...
        <Row :gutter="16">
          ...
          <Col :span="8">
            <Form.Item label="余额币种" name="currency">
              <Select
                v-model:value="accountForm.currency"
                placeholder="选择币种"
                size="large"
        ...
        <div v-if="accountForm.type === 'CUSTOM'" class="mb-4">
          <Form.Item label="自定义账户类型" required>
            <Input
              v-model:value="accountForm.customTypeName"
              placeholder="请输入账户类型，如: 基金账户、股票账户等"
            />
          </Form.Item>
        </div>
          ...
          <Row :gutter="16">
            <Col :span="12">
              <Form.Item label="币种代码" required>
                <Input
                  v-model:value="accountForm.customCurrencyCode"
            ...
            <Col :span="12">
              <Form.Item label="币种名称" required>
                <Input
                  v-model:value="accountForm.customCurrencyName"
        ...
        <div v-if="accountForm.bank === 'CUSTOM'" class="mb-4">
          <Form.Item label="自定义银行/机构名称" required>
            <Input
              v-model:value="accountForm.customBankName"
              placeholder="请输入银行或机构名称，如: 民生银行、京东金融等"
            />
          </Form.Item>
        </div>
    ...
    >
      <Form layout="vertical">
        ...
        <Form.Item label="转入账户" required>
          <Select
            v-model:value="transferForm.toAccount"
            placeholder="选择转入账户"
          >
            <Select.Option
              v-for="account in accounts.filter(
                (a) => a.id !== transferForm.fromAccount,
              )"
              :key="account.id"
              :value="account.id"
            >
    ...
    <Modal
      ...
      :footer="null"
    >
      <div v-if="accountTransactions.length === 0" class="py-12 text-center">
        <div class="mb-6 text-8xl">📊</div>
        <h3 class="mb-2 text-xl font-medium text-gray-800">暂无交易记录</h3>
      ...
      <Table
        ...
        :data-source="accountTransactions"
